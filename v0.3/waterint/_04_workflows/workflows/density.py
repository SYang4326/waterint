...
from waterint._01_core.coordinates import CoordinateSpec, coordinate_spec_from_config, coordinate_values
from waterint._01_core.selection import SelectionContext, element_indices
from waterint._01_core.species import oxygen_species_indices, oxygen_species_labels
from waterint._02_computation.density import (
    DensityResult,
    DensityState,
...
from waterint._04_workflows.workflows.common import parse_cell, parse_range, required_workflow_sections, resolve_path
...
def run_density(config: dict[str, Any]) -> DensityResult:
    ...
    if not isinstance(density_cfg, dict):
        raise ValueError("density must be a mapping when provided.")
    ...
    state = DensityWorkflowState(
        ...
        backend=str(density_cfg.get("backend", "auto")),
    )
