...
def iter_frames(traj_path: Path, input_cfg: dict[str, Any]) -> Iterator[TrajectoryFrame]:
    ...
    if fmt == "xyz":
        reader_max_frames = None
        if start_timestep is None and max_frames is not None:
            reader_max_frames = max_frames * stride
        frames = read_xyz(
            traj_path,
            reader=str(input_cfg.get("reader", "python")),
            max_frames=reader_max_frames,
        )
    elif fmt == "lammpstrj":
