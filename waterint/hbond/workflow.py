...
from waterint.common import element_indices, iter_frames, parse_cell, resolve_path, selection_context
...
def run_hbond(config: dict[str, Any]) -> HbondResult:
    ...
    counts = {species: {class_label: 0 for class_label in classes[species]} for species in species_labels}
    raw_counts = {species: {} for species in species_labels}
    samples_total = {species: 0 for species in species_labels}
    ...
    for frame in iter_frames(traj_path, input_cfg):
        ...
        for species in species_labels:
            samples_total[species] += frame_samples[species]
            for class_label in classes[species]:
                counts[species][class_label] += frame_counts[species][class_label]
            for class_label, value in frame_raw_counts[species].items():
                raw_counts[species][class_label] = raw_counts[species].get(class_label, 0) + value
        frames += 1
    ...
    fractions = {
        species: {
            class_label: (counts[species][class_label] / samples_total[species] if samples_total[species] else 0.0)
            for class_label in classes[species]
        }
        for species in species_labels
    }
    ...
    _write_hbond_csv(csv_path, counts, fractions, samples_total, classes)
    _write_raw_hbond_csv(raw_csv_path, raw_counts, counts, samples_total, classes)
    if png_path is not None:
        plot_species_labels = [
            species
            for species in species_labels
            if samples_total[species] > 0 or bool(output_cfg.get("show_empty_species", False))
        ]
        if not plot_species_labels:
            plot_species_labels = species_labels
        plot_fractions = {species: fractions[species] for species in plot_species_labels}
        plot_classes = {species: classes[species] for species in plot_species_labels}
        plot_hbond_fractions(
            path=png_path,
            fractions=plot_fractions,
    ...
    *,
    ...
    for oxygen_index, species in species_by_oxygen.items():
        if species not in counts:
            continue
        samples[species] += 1
        class_label = _topology_label(donor_counts.get(oxygen_index, 0), acceptor_counts.get(oxygen_index, 0))
...
def _find_hbonds(
    *,
    ...
    for donor_oxygen, attached_hydrogens in donor_neighbors:
        ...
        oo_vectors = _minimum_image(oxygen_positions - donor_position, cell=cell, pbc=pbc)
        ...
        for hydrogen_index in attached_hydrogens:
            ...
            for local_index in candidate_local:
                ...
                acceptor_vector = _minimum_image(
                ...
                if h_acceptor_cutoff is not None and hydrogen_acceptor_distance > h_acceptor_cutoff:
                    continue
                donor_vector = _minimum_image(
                    donor_position - hydrogen_position,
                    cell=cell,
                    pbc=pbc,
                )
...
def _topology_label(donor_count: int, acceptor_count: int) -> str:
...
def _classes_by_species(hbond_cfg: dict[str, Any], species_labels: list[str]) -> dict[str, list[str]]:
    raw = hbond_cfg.get("classes_by_species", hbond_cfg.get("classes"))
    if raw is None:
        return {
            species: list(DEFAULT_CLASSES_BY_SPECIES[species])
            for species in species_labels
        }
    if isinstance(raw, list):
        classes = _validate_classes(raw, name="hbond.classes")
        return {species: classes[:] for species in species_labels}
    ...
    for species in species_labels:
        ...
        out[species] = _validate_classes(raw_classes, name=f"hbond.classes_by_species.{species}")
    return out


def _validate_classes(raw: Any, *, name: str) -> list[str]:
